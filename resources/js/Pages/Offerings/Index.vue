        ...
        <div class="space-y-6">
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                ...
                <div class="flex items-center gap-2">
                     ...
                     <Link v-if="['admin', 'psadmin'].includes($page.props.auth.user.role)" :href="route('offerings.index')" class="bg-indigo-600 text-white px-4 py-2 rounded-lg hover:bg-indigo-700 flex items-center gap-2">
                        <Plus :size="20" /> Nuevo
                     </Link>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <Card v-for="offering in offerings.data" :key="offering.id" class="flex flex-col h-full hover:shadow-md transition">
                   ...
                   <div class="mt-4 pt-4 border-t border-slate-100">
                       <Link :href="route('referrals.create', { offering_id: offering.id })" class="block w-full text-center bg-indigo-600 text-white py-2 rounded-lg hover:bg-indigo-700 transition font-medium">
                           Referir Ahora
                       </Link>
                   </div>
                </Card>
